...
<head>
    ...
    <style>
        :root { 
            --vintage-green: #29402e; --sage-green: #485c4d; --gold: #aa8e50; 
            --cream-bg: #fdfaf2; --paper-white: #ffffff; --text-dark: #2c352d; 
        } 
...
<body>
    ...
    <script>
        async function reserverProdukt(produktNavn) {
            const navn = prompt("Indtast dit navn for reservation:");
            if (!navn) return;
        ...
        async function loadProducts(){
            ...
            products.forEach(function(p) {
                ...
                card.innerHTML = "<div><img class='product-img' src='" + (p.image_url || '') + "'><h3>" + p.name + "</h3><p>" + enhedTekst + "</p></div>" +
                                 "<div><p class='price'>" + p.price + " " + unit + "</p>" +
                                 "<button class='buy-btn' onclick='reserverProdukt(\"" + p.name + "\")'>Reserver råvare</button></div>";
